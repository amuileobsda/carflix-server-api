<?php 
    ...
    $status = $_GET['status'];
    $sg_id = $_GET['sg_id'];
    $car_info = small_car_info($con, $sg_id, $status);

    //그룹이름가져오기
    $group_name = small_group_name($con, $sg_id, $status);
?>
    ...
    <div class="flex-auto ">
      <div class="flex flex-col">
        ...
        <div class="bg-indigo-50 min-h-screen">
          ...
          <!--Table-->
          <div class="p-4 font-bold text-gray-600"><?php echo $group_name['sg_title'] ?> (<?php echo $sg_id ?>번) 차량목록</div>
            ...
            <div class="col-span-2 flex flex-auto items-center justify-between  p-5 bg-white rounded shadow-sm">
              <table class="min-w-full divide-y divide-gray-200 table-auto">
                ...
                <?php
                  ...
                  foreach($car_info['data'] as $key => $val){
                  $count++;
                ?>
                  <tr>
                    ...
                    <td class="pr-20 py-4 whitespace-nowrap text-right text-sm text-gray-500 font-medium">
                      <!-- <a href="#" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow">권한부여</a> -->
                      <a href="rent_group_vehicle_log.php?cr_id=<?php echo $val['cr_id'];?>" class="bg-blue-500 hover:bg-blue-100 text-white font-semibold py-2 px-4 border border-gray-400 rounded shadow">로그보기</a>
                    </td>
